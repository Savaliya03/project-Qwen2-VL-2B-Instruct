...
Instruction-Tuned Vision-Language Model: Leverages the Qwen2-VL-2B-Instruct model designed for multimodal tasks (text + image), enhanced with instruction tuning for more accurate responses.

Interactive Notebook: The Jupyter notebook (project_Qwen2_VL_2B_Instruct.ipynb) provides a complete walkthrough including loading the model, preparing inputs, and performing inference.

Extensible Fine-Tuning Pipeline: The qwen-vl-2b-finetune/ directory contains scripts required to adapt the model to custom datasets efficiently.
...
🧠 What is Qwen2-VL?
Qwen2-VL is a multimodal vision-language model from the Qwen family (developed by Alibaba Cloud).

“2B” refers to the model size (2 billion parameters), balancing efficiency and performance.

“Instruct” indicates that the model is specifically fine-tuned to follow human instructions effectively.
...
📜 License
This repository is publicly available under GitHub’s standard terms. The original model — Qwen2-VL-2B-Instruct — is released under the Apache License 2.0.
